...
function findWasmBinary() {
  ...
  return new URL('convertedMicroServices.wasm', import.meta.url).href;
...
function __asyncjs__get_social_graph_from_indexed_db() { return Asyncify.handleAsync(async () => { const val = Module.sharedDoc.getArray("social_graph"); return stringToNewUTF8(JSON.stringify(val)); }); }
function __asyncjs__save_user_graph_in_indexed_db(ug_json_cstr) { return Asyncify.handleAsync(async () => { const ug_json_utf_8 = UTF8ToString(ug_json_cstr); const updatedUg = JSON.parse(ug_json_utf_8); const ugArray = Module.sharedDoc.getArray("social_graph"); let idx = null; for (let i = 0; i < ugArray.length; i++) { if (ugArray.get(i).user_id == Number(updatedUg.user_id)) { idx = i; break; } } if (null != idx) { ugArray.delete(idx); ugArray.insert(idx, [updatedUg]); } else { ugArray.push([updatedUg]); } }); }
function save_follow_in_indexed_cb(cur_user_name,cur_user_id,follow_id_str,follow_username) { const curUserIdStr = UTF8ToString(cur_user_id); const curUserNameStr = UTF8ToString(cur_user_name); const followIdStr = UTF8ToString(follow_id_str); const followUsernameStr = UTF8ToString(follow_username); Module.sendFriendRequest(curUserNameStr, curUserIdStr, followIdStr, followUsernameStr); }
function edit_post_in_indexed_db(post_json_cstr) { const updatedPost = JSON.parse(UTF8ToString(post_json_cstr)); for (const userId in Module.connections) { const entry = Module.connections[userId]; if (!entry || !entry.doc) continue; const postsArray = entry.doc.getArray("posts"); for (let i = 0; i < postsArray.length; i++) { if (postsArray.get(i).post_id === Number(updatedPost.post_id)) { postsArray.delete(i); postsArray.insert(i, [updatedPost]); break; } } } }
function delete_post_in_indexed_db(post_id) { for (const userId in Module.connections) { const entry = Module.connections[userId]; if (!entry || !entry.doc) continue; const postsArray = entry.doc.getArray("posts"); for (let i = 0; i < postsArray.length; i++) { if (postsArray.get(i).post_id === Number(post_id)) { postsArray.delete(i); break; } } } }
function get_posts_from_indexed_db() { const aggregatedPosts = []; for (const userId in Module.connections) { console.log("connection for userid" + userId); const entry = Module.connections[userId]; if (!entry || !entry.doc) continue; const postsArray = entry.doc.getArray("posts"); for (const post of postsArray.toArray()) { aggregatedPosts.push(post); } } console.log("posts in index" + aggregatedPosts.length + " " + JSON.stringify(aggregatedPosts)); return stringToNewUTF8(JSON.stringify(aggregatedPosts)); }
function save_post_in_indexed_db(post_json_cstr) { const post = JSON.parse(UTF8ToString(post_json_cstr)); if (!Module.connections) { return; } for (const userId in Module.connections) { const connection = Module.connections[userId]; if (!connection || !connection.doc) continue; const postsArray = connection.doc.getArray("posts"); postsArray.push([post]); console.log(`Post saved for user ${userId}`); } }
function get_user_in_session_storage_js() { const user_json_str = sessionStorage.getItem('user'); return stringToNewUTF8(user_json_str); }
function add_user_in_session_storage_js(user_json_cstr) { const user_json_utf_8 = UTF8ToString(user_json_cstr); sessionStorage.setItem('user', user_json_utf_8); const userJson = JSON.parse(user_json_utf_8); const username = userJson["username"]; const userId = userJson["userid"]; Module.mainProvider.awareness.setLocalStateField('user', { username: username, userId: userId, clientId: Module.sharedDoc.clientID, }); }
function remove_user_in_session_storage_js() { sessionStorage.removeItem('user'); }
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['dynCall_iiiijii'] != 'undefined', 'missing Wasm export: dynCall_iiiijii');
  ...
  assert(typeof wasmExports['dynCall_viijjj'] != 'undefined', 'missing Wasm export: dynCall_viijjj');
  ...
  assert(typeof wasmExports['dynCall_viijjji'] != 'undefined', 'missing Wasm export: dynCall_viijjji');
  ...
  dynCall_iiiijii = dynCalls['iiiijii'] = createExportWrapper('dynCall_iiiijii', 7);
  ...
  dynCall_viijjj = dynCalls['viijjj'] = createExportWrapper('dynCall_viijjj', 6);
  ...
  dynCall_viijjji = dynCalls['viijjji'] = createExportWrapper('dynCall_viijjji', 7);
